            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                      ...
                      data-url="{{ route('addref_doctor') }}" data-size="md" title="Edit Area" class="load-popup btn btn-primary mx-2  "> 
...
<?php $service['Ref_doctorID']=$item['UserID'] ?>
<?php $service['Ref_doctorData']=$item ?>
{{-- {{ $service['Ref_doctorData']=$item }} --}}
              <a href="javascript:;" data-real="{{ $item['UserID'] }}" 
              data-param="{{openssl_encrypt(json_encode($item),"AES-128-ECB",md5(env('ENC_SALT'))) }}" 
              data-url="{{ route('addref_doctor') }}" data-size="md" title="Edit Area" 
