            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                      ...
                      data-url="{{ route('addpatient') }}" data-size="md" title="Edit Area" class="load-popup btn btn-primary mx-2  "> 
...
<?php $service['PatientID']=$item['UserID'] ?>
<?php $service['PatientData']=$item ?>
{{-- {{ $service['PatientData']=$item }} --}}
              <a href="javascript:;" data-real="{{ $item['UserID'] }}" 
              data-param="{{openssl_encrypt(json_encode($item),"AES-128-ECB",md5(env('ENC_SALT'))) }}" 
              data-url="{{ route('addpatient') }}" data-size="md" title="Edit Area" 
